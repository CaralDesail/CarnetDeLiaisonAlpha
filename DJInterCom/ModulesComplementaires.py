...
def callOfCorrespondantList(request, CorrespondingMessage,
                            messageID):  # return a string with the name of correspondants
    try:
        listCorrespondants = []
        for correspondant in (CorrespondingMessage.accessUserID).split(";"):
            Correspondant = str(User.objects.get(pk=correspondant).first_name) + " " + str(
                User.objects.get(pk=correspondant).last_name)
...
def correspondanceTableMessages_check(
        request):  # will check if corresponding item in correspondance table exist, if not create
    liste_new = []
...
def add_sentToCorrespondanttoAccessList():
    ...
    for idTOCheck in IDCorrList:
        ...
        ListOfUsersIntable = StringOfUsersinTable.split(
            ";")  # gives the list of access to messages in correspondance table
